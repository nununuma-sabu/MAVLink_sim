...
## 将来の追加機能
- [x] ミッション計画（ウェイポイント自動飛行）
- [ ] 複数ドローン対応
- [ ] センサーシミュレーション（カメラ等）
- [ ] 3Dビューの強化（地形、建物等）
- [ ] 3D描画のモダンOpenGL化
  - [x] 建物・道路・線路・地面を静的都市メッシュとして分離
  - [x] 静的都市メッシュをVBO化
  - [x] シェーダーによる頂点色・フォグ・基本ライティングへ移行
  - [x] 飛行トレース、ウェイポイント、ドローンモデルを動的VBO化
    - [x] 飛行トレースとウェイポイント経路を動的VBO化
    - [x] ウェイポイントマーカーを動的VBO化
    - [x] ドローンモデルを動的VBO化
  - 参考: `docs/modern-opengl-migration.md`
